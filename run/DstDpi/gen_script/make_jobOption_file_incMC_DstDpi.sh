...
COUNT=0
for ECM in ${ECMS[@]}; do
    NUM_UP=$(ls -l /besfs/groups/cal/dedx/$USER/bes/DstDpi/run/DstDpi/samples/mc/$TYPE/$ECM | grep "txt" | wc -l)
    JobText_SaveDir=/besfs/groups/cal/dedx/$USER/bes/DstDpi/run/DstDpi/jobs_text/mc/$TYPE/$ECM
    ROOT_SaveDir=/besfs/groups/cal/dedx/$USER/bes/DstDpi/run/DstDpi/rootfile/mc/$TYPE/$ECM
    ...
    do
        file_list=incMC_${TYPE}_${ECM}_DstDpi_20G-${num}.txt
        rootfile=DstDpi_incMC_${TYPE}_$ECM-${num}.root
        jobOptions=jobOptions_DstDpi_incMC_${TYPE}_$ECM-${num}.txt
        ...
        echo "#include \"/besfs/groups/cal/dedx/\$USER/bes/DstDpi/run/DstDpi/samples/mc/$TYPE/$ECM/$file_list\" " >> ${JobText_SaveDir}/${jobOptions}
