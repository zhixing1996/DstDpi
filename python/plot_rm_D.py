...
__created__ = "[2020-05-11 Mon 10:32]"
...
SYNOPSIS
    ./plot_rm_D.py [ecms] [D_sample] [D_type]
...
def rm_D_fill(t, h, D_sample, D_type):
...
def plot(path, leg_title, ecms, xmin, xmax, xbins, D_sample, D_type):
    ...
    try:
        ...
        logging.info('D2(2460) entries :'+str(entries_D2))
    except:
    ...
    if D_sample == 'D0' or (D_sample == 'Dplus' and D_type == 'Dst'):
        pt = TPaveText(0.7, 0.65, 0.85, 0.85, "BRNDC")
        set_pavetext(pt)
        pt.Draw()
    ...
    if D_sample == 'Dplus' and D_type == 'D':
        pt = TPaveText(0.65, 0.65, 0.85, 0.85, "BRNDC")
        set_pavetext(pt)
        pt.Draw()
        pt.AddText(leg_title)
        pt.AddText('e^{+}e^{-}#rightarrowD^{+}#pi_{0}^{-}#bar{D^{0}}')
    ...
    mbc.SaveAs('./figs/rm_D_'+str(ecms)+'_'+D_sample+'_'+D_type+'.pdf')
